...
class CommunityService extends Component {
	...
	displayImages = () => {
		return this.state.photos.map( (pic, index) => 
			<Grid.Column computer={4} mobile={16} tablet={8} style={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
				<a className={`instaLink instaImage${index}`} href="https://www.instagram.com/fit_2recover_service/">
				<div style={styles.instaPics}>
		      		<Image src={pic.images.standard_resolution.url} style={{maxHeight: '300px', borderRadius: '30px'}}/>
		    </div>
		   </a>
			</Grid.Column>
		)
...
<div>
		...
		<div className="whatWeDo" style={styles.whatWeDo}>
			...
			<div className="whatWeDoRightWomens">
				...
				It is crucial to form bonds and friendships with other women in recovery.<br /><br /> Thursdays: 7-8pm @ FTR<br /><br /> <a href="mailto:contact@fit2recover.org" >Click here for more information on Woman's Group!</a></div>
			</div>
		</div>
		...
		<Grid className="instaGrid" style={{height: '100%', width: '100%', margin: '0', justifyContent: 'space-around'}}>
			{ this.displayImages() }
		</Grid>	
